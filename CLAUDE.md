...
- `EDGE_SHARED_SECRET` (Production) — the `verify-submission` route sends it as `x-edge-secret` to the email edge function; **must match** the Supabase Edge Function secret + Vault `edge_shared_secret`. (Legacy `MAILGUN_API_KEY`/`MAILGUN_DOMAIN` Vercel vars are **unused** — the edge function reads its own copies from Supabase Edge Function secrets.)
...
- **Mailgun SMTP is live** in Auth → SMTP Settings: sending subdomain `mg.kitchenequipment.ca`, credential `no-reply@mg.kitchenequipment.ca`, `smtp.mailgun.org:587` (US region). Magic-link emails send live; custom-SMTP rate limit raised to 30/hr (past the dev shared-SMTP ~2/hr cap).
...
- **Email (Mailgun) — all three paths live (PR #67):** magic-link login (Auth custom SMTP) + admin notifications + applicant receipts (the `send-transactional-email` edge function). All send from `no-reply@mg.kitchenequipment.ca` (dedicated sending subdomain; **DNS is at GoDaddy**, migrated from Atom.com). **Two hard rules:** (1) the edge function **must stay `verify_jwt: false`** (pinned in `supabase/config.toml`) — pg_net triggers call it with no JWT, so re-enabling JWT 401s every notification email at the gateway; (2) `EDGE_SHARED_SECRET` must be the **same value in 3 stores** — Supabase Edge Function secret, Vault `edge_shared_secret`, Vercel env. The Mailgun tracking CNAME (`email.mg`) is deliberately **omitted** (click-tracking rewrites would let scanners consume magic-link one-time tokens). Full setup + values: `project_dns_and_email_infra` memory + SESSION-LOG.
...
2. **SEC-05 — Rate-limit + Turnstile on `/api/review` & `/api/contact`.** Honeypot-only today. Each review insert fires an admin notification email → **email-bomb / Mailgun-quota vector, now ACTIVE since Mailgun went live (PR #67)** (was latent while the secret was unset — this is now the most pressing backlog item). Mirror `list-company.ts`: Turnstile (`verifyTurnstile`/`parseTurnstileResponse` in `src/lib/validation.ts`), per-IP limit (~5/10 min via `getClientIp` + service-role count), length caps (names ≤120, bodies ≤5000).
...
**Edge Function note (PR #39 + #67):** `send-transactional-email` refuses unauthenticated calls when `EDGE_SHARED_SECRET` is unset on a hosted deploy (open-relay guard) and runs `verify_jwt:false` (pinned in `supabase/config.toml`) so the no-JWT pg_net triggers can reach it. Mailgun is now **wired + live** — `EDGE_SHARED_SECRET` is set across all three stores (Edge secret + Vault + Vercel). Edge functions don't deploy via git: `config.toml`/secret changes take effect on the next `supabase functions deploy send-transactional-email`.
...
| 2026-06-08 | #67 | **Mailgun email go-live.** DNS records handed to the owner (no shared access) at **GoDaddy** (untangled an in-flight Atom.com→GoDaddy nameserver migration + a browser-cached parking-redirect red herring); `mg.kitchenequipment.ca` sending subdomain (SPF/DKIM-2048/MX/DMARC; tracking CNAME omitted for magic-link token safety); Auth custom SMTP → magic-link login; `send-transactional-email` wired for admin notifications + applicant receipts. **Key fix: `verify_jwt:false`** (pg_net triggers call with no JWT → gateway was 401-ing every notification) now pinned in new `supabase/config.toml`. Removed orphaned `notify-submission` edge fn. All 3 paths inbox-verified, sending from `no-reply@`. |
| 2026-06-09 | #68 | GA4 (`G-LR39F9SB25`) + GSC verification meta added to `Base.astro` — site-wide; verified live via Chrome DevTools (`dataLayer` config event confirmed) |
| 2026-06-11 | #69 | **Blog re-enabled.** Humanized content (Bryan Johnston, AI score ≤20%) applied via migration 018 to all 8 posts; new 9th post added (`best-commercial-kitchen-equipment-suppliers-toronto`). Removed `noindex` from `/blog` + `/blog/[slug]`, restored Blog nav links (desktop + mobile), dropped blog from sitemap filter. |
